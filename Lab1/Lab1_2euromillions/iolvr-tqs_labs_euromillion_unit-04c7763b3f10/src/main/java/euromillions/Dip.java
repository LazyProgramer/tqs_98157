...
public class Dip {
    ...
    final static int MAXNUMSTARS = 2;
    final static int MAXNUMNUMBERS = 5;
    final static int MAXSTARVAL = 11;
    final static int MAXNUMBERVAL = 49;
    ...
    public Dip(int[] arrayOfNumbers, int[] arrayOfStarts) {
        ...
        if (MAXNUMNUMBERS == arrayOfNumbers.length && MAXNUMSTARS == arrayOfStarts.length) {
    ...
    public static Dip generateRandomDip() {
        ...
        for (int i = 0; i < MAXNUMNUMBERS; ) {
            int candidate = generator.nextInt(MAXNUMBERVAL) + 1;
            if (!randomDip.getNumbersColl().contains(candidate)) {
                randomDip.getNumbersColl().add(candidate);
                i++;
            }
        }
        for (int i = 0; i < MAXNUMSTARS; ) {
            int candidate = generator.nextInt(MAXSTARVAL) + 1;
